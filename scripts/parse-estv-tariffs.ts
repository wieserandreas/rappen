 ...
 *
 * Run: pnpm tsx scripts/parse-estv-tariffs.ts
 *
 * Input:  data/qst-2026/extracted/tar26{canton}.txt (CRLF, ASCII, fixed-width)
 * Output: packages/swiss-data/src/withholding-tax/tariffs/2026/{CANTON}.json
 */
...
const INPUT_DIR = join(ROOT, "data/qst-2026/extracted");
...
const OUTPUT_DIR = join(ROOT, "packages/swiss-data/data/qst-2026");
...
function main() {
	...
	const files = readdirSync(INPUT_DIR)
		.filter((f) => /^tar26[a-z]{2}\.txt$/.test(f))
		.sort();

	if (files.length === 0) {
		console.error(`❌ No tar26??.txt files found in ${INPUT_DIR}`);
		process.exit(1);
	}
	...
	for (const file of files) {
		const cantonLower = file.substring(5, 7);
